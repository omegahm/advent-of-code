...
def score(char)
  char.ord - (/[a-z]/ === char ? 96 : 38)
end
# PART 1
result = input.lines.map(&:chomp).map do |line|
  line1, line2 = line.slice(0, line.size / 2), line.slice(line.size / 2, line.size)
  char = (line1.chars & line2.chars).join
  score(char)
end
...
# PART 2
result = input.lines.map(&:chomp).each_slice(3).map do |line1, line2, line3|
  char = (line1.chars & line2.chars & line3.chars).join
  score(char)
end

...
def move_tail(visited, head_pos, tail_pos)
  hx, hy = head_pos
  tx, ty = tail_pos

  return if NEIGHBOURS.any? { |dx, dy| hx + dx == tx && hy + dy == ty }
...
def move_tails(visited, head_pos, tails_pos)
  move_tail([], head_pos, tails_pos[0])
  7.times do |i|
    move_tail([], tails_pos[i], tails_pos[i+1])
  end
  move_tail(visited, tails_pos[-2], tails_pos[-1])
end

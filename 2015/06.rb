...
input.lines.each do |line|
  a, b, c, d = line.scan(/\d+,\d+/).flat_map { _1.split(",").map(&:to_i) }
  ...
  when /^turn/
    ...
    a.upto(c) do |x|
      b.upto(d) do |y|
        lights[[x, y]][0] = bool
        lights[[x, y]][1] = [lights[[x, y]][1] + (bool ? 1 : -1), 0].max
      end
    end
  when /^toggle/
    a.upto(c) do |x|
...
puts "Part 1: #{lights.values.map(&:first).count(true)}"
puts "Part 2: #{lights.values.map(&:last).sum}"

...
rules_string, parts = input.split("\n\n")
...
rules_string.split("\n").map do |rule|
  ...
  rules[name] = rest.split(",").map do |rule_part|
    operation, result = rule_part.split(":")
    case operation
    when /[xmas][<>]/
      { op: operation[0], comp: operation[1], value: operation[2..].to_i, result: result }
    else
      { op: operation }
    end
  end
...
parts = parts.split("\n").map do |part|
  part = part[1..-2].split(",")
  part.each_with_object({}) do |obj, hash|
    obj.split("=").tap do |key, value|
      hash[key] = value.to_i
    end
  end
end
...
parts.each do |part|
  ...
  loop do
    ...
    break if current_rule == "R"

    rules[current_rule].each do |rule|
      case rule[:op]
      when "x", "m", "a", "s"
        if part[rule[:op]].public_send(rule[:comp], rule[:value])
          current_rule = rule[:result]
          break
        end
      else
        current_rule = rule[:op]
        break
      end
    end
  end
...
puts "Part 1: #{accepted.flat_map(&:values).sum}"
...
parts = []
parts << { "x" => (1..4000), "m" => (1..4000), "a" => (1..4000), "s" => (1..4000), "rule" => "in" }

until parts.empty?
  part = parts.pop
  current_rule = part["rule"]

  if current_rule == "A"
    accepted << part.except("rule")
    next
  elsif current_rule == "R"
    next
  end
  ...
  rules[current_rule].each do |rule|
    ...
    when "x", "m", "a", "s"
      other = part.dup
      other["rule"] = rule[:result]
      range = part[xmas]

      case rule[:comp]
      when "<"
        other[xmas] = (range.begin..rule[:value]-1)
        part[xmas] = (rule[:value]..range.end)
      when ">"
        other[xmas] = (rule[:value]+1..range.end)
        part[xmas] = (range.begin..rule[:value])
      end
...
puts "Part 2: #{accepted.map { |part| part.values.reduce(1) { |acc, h| acc *= h.size } }.sum}"

...
def parse_map(input)
  map = input.lines.map do |line|
    line.chomp.tr("|7JLF-", "│┐┘└┌─").chars.zip([nil] * line.size)
  end
...
def solve(input, start)
  ...
  loop do
    i = 0
    count += 1
    n1, n2 = find_next_xy(map, x1, y1)
    ny1, nx1 = n1
    ny2, nx2 = n2

    begin
      if !map[ny1][nx1][1].nil? && !map[ny2][nx2][1].nil?
        # We've searched both paths and found the midpoint
        return map
      elsif map[ny1][nx1][1].nil?
        map[ny1][nx1][1] = count
        x1, y1 = nx1, ny1
      elsif map[ny2][nx2][1].nil?
        map[ny2][nx2][1] = count
        x1, y1 = nx2, ny2
      else
        raise "WHAT?"
...
puts "Part 1: #{result.flatten(1).map(&:last).compact.max}"

enclosed_map = result.map { |row| row.map { |a, b| b ? a : "@" } }
...
loop do
  ...
  enclosed_map.each_with_index do |row, y|
    row.each_with_index do |cell, x|
      ...
      if cell == "@"
        if [
          ...
          [y, x-1],
          [y, x+1],
        ].any? { |ny, nx| ny >= 0 && nx >= 0 && ny < enclosed_map.size && nx < row.size && enclosed_map[ny][nx] == " " }
          done = false
          enclosed_map[y][x] = " "
        end
      end
...
enclosed_map.each_with_index do |row, y|
  ...
  row.each_with_index do |cell, x|
    ...
    when "│", "┘", "└"
      count += 1
    when "@"
      enclosed_map[y][x] = count.even? ? " " : "@"
    end
  end
end

enclosed_map.each do |row|
  puts row.join.gsub(" ", "\033[106m \033[0m").gsub("@", "\033[91m@\033[0m")
end
puts "Part 2: #{enclosed_map.flatten.count("@")}"

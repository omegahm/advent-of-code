...
def karger_min_cut(vertices_arg, edges_arg)
  loop do
    vertices = vertices_arg.dup
    edges = edges_arg.dup.shuffle
    ...
    while vertices.size > 2
      ...
      edges.reject! { |(from, to)| from == to }
    end

    return vertices if edges.size == 3
  end
end

edges = input.split("\n").each_with_object([]) do |line, array|
  from, to = line.split(": ")
  to.split(" ").each do |adj|
    array << [from, adj]
  end
end
vertices = edges.flatten.uniq.each_with_object({}) { |v, hash| hash[v] = 1 }

puts "Part 1: #{karger_min_cut(vertices, edges).values.reduce(:*)}"

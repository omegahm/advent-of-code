...
def score(line, part)
  max = { "red" => part == 1 ? 12 : 0, "green" => part == 1 ? 13 : 0, "blue" => part == 1 ? 14 : 0 }

  line.gsub(/Game \d+: /, "").split("; ").each do |game|
    game.split(", ").each do |draw|
      amount, color = draw.split
      return 0 if part == 1 && max[color] < Integer(amount)

      max[color] = [max[color], Integer(amount)].max
    end
  end

  if part == 1
    Integer(line.match(/Game (\d+)/)[1])
  else
    max.values.reduce(:*)
  end
end

puts input.lines.sum { |line| score(line, 1) }
puts input.lines.sum { |line| score(line, 2) }

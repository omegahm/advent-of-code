...
input.lines.each_with_index do |line, i|
  line.chars.each_with_index do |char, j|
    if /\d/ === char
      numbers[-1] << [i, j, char, false]
    else
      if !numbers.last.empty?
        numbers << []
      end
...
def neighbour?(num, i, j)
  num.any? do |(a, b)|
    Array(-1..1).repeated_permutation(2).any? { |(x, y)| a == i + x && b == j + y }
  end
end

gear_ratio = 0
symbols.each do |(i, j, char)|
  next unless /[^\d.\n]/ === char

  if char == "*"
    selected_numbers = numbers.select { |num| neighbour?(num, i, j) }

    if selected_numbers.size == 2
      gear_ratio += selected_numbers.map { |num| Integer(num.map { _3 }.join) }.reduce(:*)
    end
  end

  numbers.map! do |num|
    num.map! { [_1, _2, _3, _4 || neighbour?(num, i, j)] }
  end
end

result = numbers.select { _1.map(&:last).any? }.map do |num|
  Integer(num.map { _3 }.join)
end.sum

puts "Part 1: #{result}"
puts "Part 2: #{gear_ratio}"

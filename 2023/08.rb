...
directions = input.lines.first.strip.chars.cycle
map = input.lines.drop(2).map { |l| from, to = l.split(' = '); [from, to.delete("()").split(",").map(&:strip)] }.to_h

current = "AAA"
count = 0
until current == "ZZZ"
  count += 1
  current = map[current][directions.next == "L" ? 0 : 1]
end
puts "Part 1: #{count}"
...
as = map.keys.select { /..A/ === _1 }
distances = map.keys.map { |k| [k, 0] }.to_h
...
puts "Part 2: #{as.map { distances[_1] }.reduce(&:lcm)}"

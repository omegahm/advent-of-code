...
rooms = input.split("\n").map do |line|
  encrypted, sector_id, checksum = line.match(/([a-z-]+)-(\d+)\[([a-z]+)\]/).captures
  [encrypted, sector_id.to_i, checksum]
end

def valid_room?(room)
  encrypted, sector_id, checksum = room
...
def decrypt_room(room)
  encrypted, sector_id, _ = room

  encrypted = encrypted.chars.map do |char|
    if char == "-"
      " "
    else
      ((char.ord - "a".ord + sector_id) % 26 + "a".ord).chr
    end
  end

  [encrypted.join, sector_id]
end

valid_rooms = rooms.select { valid_room?(it) }
puts "Part 1", valid_rooms.sum { it[1] }
puts "Part 2", valid_rooms.map { decrypt_room(it) }.find { it[0].include?("north") }[1]

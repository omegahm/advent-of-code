...
wire_a = input.split("\n")[0].split(",").map { [_1[0], _1[1..-1].to_i] }
wire_b = input.split("\n")[1].split(",").map { [_1[0], _1[1..-1].to_i] }

i, x, y = [1, 0, 0]
wire_a.each do |direction, distance|
  distance.times do
    case direction
    when "R"
      x += 1
...
i, x, y = [1, 0, 0]
wire_b.each do |direction, distance|
  distance.times do
    case direction
    when "R"
      x += 1
    ...
    if grid[[x, y]] && grid[[x, y]][0] == "A"
      intersections << [x, y, i + grid[[x, y]][1]]
    elsif grid[[x, y]].nil?
      grid[[x, y]] = ["B", i]
    end
...
result = intersections.map do |x, y, _i|
  x.abs + y.abs
end.min

puts "Part 1: #{result}"

puts "Part 2: #{intersections.map { _1[2] }.min}"
